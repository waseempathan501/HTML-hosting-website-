...
PROJECTS_DIR = os.path.join(UPLOAD_DIR, "hosted_projects")

if not os.path.exists(PROJECTS_DIR):
    os.makedirs(PROJECTS_DIR)

# Automatic image finder route (Checks multiple common names)
@app.route('/image')
def profile_image():
    possible_names = [
        "1768110605494.jpg",
    ...
    # Agar inme se koi bhi file milti hai toh wo serve kar dega, warna folder ki pehli jpg utha lega
    for name in possible_names:
        if os.path.exists(os.path.join(UPLOAD_DIR, name)):
            return send_from_directory(UPLOAD_DIR, name)
    
    # Fallback: find any .jpg file in the directory
...
<head>
    ...
    <title>WASEEM PRO HACKER - Universal Portal</title>
    <style>
    ...
    <div class="neon-box">
        <div class="content-container">
            ...
            <div class="footer-note">
                🔥 ⚡ ★𝗰𝗿𝗮𝗳𝘁𝗲𝗱 𝘄𝗶𝘁𝗵 𝗽𝗿𝗲𝗰𝗶𝘀𝗶𝗼𝗻 𝗯𝘆 𝘄𝗮𝘀𝗲𝗲𝗺 — 𝗯𝘂𝗶𝗹𝘁 𝗳𝗼𝗿 𝗵𝗶𝗴𝗵-𝗲𝗻𝗱 𝘀𝗲𝗰𝘂𝗿𝗶𝘁𝘆 𝘁𝗲𝘀𝘁𝗶𝗻𝗴, 𝗰𝘂𝘀𝘁𝗼𝗺 𝗵𝗼𝘀𝘁𝗶𝗻𝗴, 𝗮𝗻𝗱 𝗻𝗲𝘅𝘁- 𝗴𝗲𝗻𝗲𝗿𝗮𝘁𝗶𝗼𝗻.★ 🔥
            </div>
...
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Waseem Hosting Panel</title>
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; }
        body {
            background-color: #05050a;
...
<body>
    <div class="neon-box">
        <div class="content-container">
            <div class="creator-banner">⚡ SECURE DEPLOYMENT ENGINE ⚡</div>
            <h2>Waseem Hosting Panel</h2>
            
            <form method="POST" enctype="multipart/form-data">
                <div class="form-group">
                    <label>PROJECT NAME (SLUG)</label>
                    <input type="text" name="project_name" placeholder="e.g. mytool" required>
                </div>
                <div class="form-group">
                    <label>UPLOAD HTML FILE</label>
                    <input type="file" name="project_file" required>
                </div>
                <button type="submit">Deploy Project Instantly</button>
            </form>

            {% if success_url %}
            <div class="alert">
                🚀 Deployed Successfully!<br>Live Link: <a href="{{ success_url }}" target="_blank">{{ request.host_url[:-1] }}{{ success_url }}</a>
            </div>
            {% endif %}

            <div class="footer-extra">
                Advanced cloud infrastructure powered by custom server scripts. Fast, secure, and always online.
            </div>
...
def hosting():
    ...
    if request.method == 'POST':
        ...
        if project_name and project_file:
            ...
            success_url = f"/view/{project_name}"

    return render_template_string(HOSTING_TEMPLATE, success_url=success_url)

@app.route('/view/<project_name>')
def view_project(project_name):
    file_path = os.path.join(PROJECTS_DIR, project_name, "index.html")
    if os.path.exists(file_path):
        with open(file_path, 'r', encoding='utf-8') as f:
            return f.read()
    return "<h3 style='font-family:sans-serif; text-align:center; margin-top:50px; color:#ff5722;'>404 - Project Not Found!</h3>", 404
